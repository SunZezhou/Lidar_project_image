    ...
    void projector::projection_callback(const sensor_msgs::Image::ConstPtr &img, 
                                    const sensor_msgs::PointCloud2::ConstPtr &pc)
        {
            ...
            for(pcl::PointCloud<pcl::PointXYZI>::const_iterator it = cloud->points.begin(); it != cloud->points.end(); it++){
                ...
                cv::Mat rt_inv(4,4,cv::DataType<double>::type);
                cv::invert(i_params.RT,rt_inv);
                Y=i_params.cameraIn* rt_inv * X;
                // std::cout<<Y<<std::endl;
                // std::cout<<Y.at<double>(2,0)<<std::endl;

                cv::Point pt;
                pt.x = Y.at<double>(0,0) / Y.at<double>(0,2);
                pt.y = Y.at<double>(1,0) / Y.at<double>(0,2);

                if(0<= pt.x <= 1280&& 0<=pt.y<=1024)
                {
                float val = it->x;
                float maxVal = 18.0;
            ...
            cv_ptr->header.frame_id = "velodyne";
